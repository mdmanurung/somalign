...
Cross-panel alignment by Gromov-Wasserstein OT: matches intra-codebook distance
structures, so query and reference need not share markers (experimental; balanced
GW). Returns a query-node-by-reference-node coupling (with exact marginals).

```{r fit-gw, eval = requireNamespace("kohonen", quietly = TRUE)}
gw <- somalign_fit_gw(qry, ref, epsilon = 0.05)
stopifnot(inherits(gw, "somalign_gw_fit"))
stopifnot(all.equal(rowSums(gw$correspondence),
                    rep(1, nrow(gw$correspondence)), tolerance = 1e-6))
gw
```
...
## `somalign_mapping_confidence`

Per-cell reference-mapping confidence in `(0, 1]` (a continuous complement to the
boolean outside-reference flag).

```{r mapping-confidence, eval = requireNamespace("kohonen", quietly = TRUE)}
mc <- somalign_mapping_confidence(fit, k = 3)
stopifnot(length(mc) == nrow(new_data), all(mc > 0 & mc <= 1))
cat("Mapping confidence range:", paste(round(range(mc), 3), collapse = " - "), "\n")
```
...
## `somalign_conformal_labels`

Conformal prediction sets over per-cell class probabilities, with a
distribution-free coverage guarantee.

```{r conformal}
classes <- c("pop_A", "pop_B")
gen <- function(n) {
  y <- sample(classes, n, replace = TRUE)
  p <- matrix(runif(n * 2, 0, 0.4), n, 2, dimnames = list(NULL, classes))
  i <- cbind(seq_len(n), match(y, classes))
  p[i] <- p[i] + 0.6
  list(p = p / rowSums(p), y = y)
}
cal <- gen(200); qry <- gen(50)
cp <- somalign_conformal_labels(qry$p, cal$p, cal$y, alpha = 0.1)
stopifnot(inherits(cp, "somalign_conformal"))
cp
```
